...
namespace ShopOn.Web.Data;

public static class EasyInventoryReportingExtensions
{
    /// <summary>LINQ replacement for SQL Server stored procedure spSOReport (not ported to PostgreSQL).</summary>
    public static IEnumerable<spSOReport_Result> spSOReport(this EasyInventoryDbContext db, string saleOrderID)
    {
        ...
        var biz = db.MyBusinessInfos.AsNoTracking().FirstOrDefault();
        ...
        foreach (var sod in so.SODs)
        {
            ...
            if (sod.ProductId.HasValue)
                prod = db.Products.AsNoTracking().FirstOrDefault(p => p.Id == sod.ProductId.Value);
    ...
    public static IEnumerable<spPOReport_Result> spPOReport(this EasyInventoryDbContext db, string purchaseOrderID)
    {
        ...
        foreach (var pod in po.PODs)
        {
            ...
            if (pod.ProductId.HasValue)
                prod = db.Products.AsNoTracking().FirstOrDefault(p => p.Id == pod.ProductId.Value);
